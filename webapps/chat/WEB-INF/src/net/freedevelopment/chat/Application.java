...
import net.freedevelopment.chat.ClientManager;
...
//		 test if username is already used
	...
	public void send_private(String fromPseudo, String DestinationID,String msg) {
		...
		//Iterator<IConnection> it = scope.getConnections();
		//Iterator<IConnection> it = (Iterator) scope.getConnections();
		Collection<Set<IConnection>> high=scope.getConnections();
		Iterator<Set<IConnection>> ithigh=high.iterator();
		while(ithigh.hasNext())
		{
			Set<IConnection> middle=ithigh.next();
			Iterator<IConnection> itmiddle=middle.iterator();
			while(itmiddle.hasNext())
			{
				IConnection conn=itmiddle.next();
				String id=conn.getClient().getId();
